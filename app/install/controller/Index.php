...
class Index extends BaseController
{
    ...
    public function __construct(App $app)
    {
        ...
        $this->config = [
            'siteName' => 'east',
            'siteVersion' => config('app_version'),
            'tablePrefix' => 'fun_',
    ...
    public function step3()
    {
        ...
        if (request()->action() == 'step3' && request()->isPost()) {
            ...
            $db['host'] = request()->post('hostname') ?: '127.0.0.1';
            ...
            if (!$link->select_db($db['database'])) {
                $create_sql = 'CREATE DATABASE IF NOT EXITS' . $db['database'] . 'DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;';
            ...
            if (!$putConfig) {
                $this->error('安装失败，请确认database.php是否有写入权限');
            }
            if ($this->app_debug) {
                $putEnv = str_replace(
                ...
                if (!$putConfig) {
                    $this->error('安装失败,请确认安装目录是否有写入权限');
                }
                //设置lockFile访问和修改时间
                $result = @touch($this->lockFile);
                if (!$result) {
                    $this->error('安装失败，请去人install.lock是否有写入权限');
                }
                try {
                    $instance = Db::connect();
                    ...
                    foreach ($sqlFiles as $k => $v) {
                        ...
                        if (!$sql) {
                            $this->error("无法读取{$v}文件，请检查是否有读权限");
                        }
